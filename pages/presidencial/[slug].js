...
export default function PerfilPresidenciavel({ candidato, colega, canonical }) {
  ...
  return (
    <div className="pagina">
      ...
      {/* Coluna única, largura padrão do site (mesma de outras páginas de conteúdo) — o plano de
          governo vem abaixo do resto, não mais lado a lado, e usa a largura toda pra respirar. */}
      <div style={{ maxWidth: t.larguraMax, margin: '0 auto' }}>
        ...
        {/* Proposta de governo — sem resumo por IA por padrão (decisão do Jordy, 2026-08-20): link
            direto pro PDF oficial. Agora vem ABAIXO do resto (não mais numa coluna lateral) e usa
            a largura toda — os temas do resumo, quando existem, se organizam em grade. */}
        <section style={{ background: t.cor.verde, borderRadius: t.raio.lg, padding: 'clamp(20px,3.4vw,32px)', marginBottom: '20px' }}>
          <h2 style={{ fontFamily: t.fonte.titulo, fontWeight: 600, fontSize: '1.3rem', color: '#fff', margin: '0 0 16px' }}>Plano de governo</h2>
          {c.proposta_pdf_url ? (
            <>
                      ...
                      <div key={bloco.tema}>
                        ...
                        <ul style={{ margin: 0, paddingLeft: '18px', display: 'grid', gap: '4px' }}>
                          {(bloco.pontos || []).map((p, i) => (
                            <li key={i} style={{ color: 'rgba(255,255,255,0.85)', fontSize: '0.87rem', lineHeight: 1.45 }}>{p}</li>
                          ))}
                        </ul>
                      </div>
                    ))}
                  </div>
                  <p style={{ color: 'rgba(255,255,255,0.55)', fontSize: '0.74rem', lineHeight: 1.4, margin: '0 0 16px' }}>
                    Resumo gerado por IA a partir do PDF oficial, sem opinião, pode conter imprecisões. Leia o documento completo pra conferir.
                  </p>
                </>
              ) : (
                <p style={{ color: 'rgba(255,255,255,0.78)', fontSize: '0.88rem', lineHeight: 1.5, margin: '0 0 16px' }}>
